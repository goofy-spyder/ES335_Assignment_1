...
def information_gain(Y: pd.Series,Y_low: pd.Series, Y_high:pd.Series, criterion: None) -> float:
    ...
    # print(Y_low)
    gain = 0
    w_low = len(Y_low) / len(Y)
    w_high = 1 - w_low
    if(criterion == "gini_index"):
        gain = gini_index(Y) - (w_low* gini_index(Y_low) + w_high*gini_index(Y_high))
    elif (criterion == "information_gain"):
        gain  = (entropy(Y) - (w_low*entropy(Y_low) + w_high*entropy(Y_high)))
    else:
